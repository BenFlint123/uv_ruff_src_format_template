...
5. **Review `.github/CODEOWNERS`** — currently lists Ben Flint as template owner. Replace with new repo owner's GitHub ID.
...
```powershell
git grep -i "newpackagename"          # should match only the new name
git grep -i "mypackage"        # should return nothing
```
